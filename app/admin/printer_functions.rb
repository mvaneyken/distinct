ActiveAdmin.register PrinterFunction do
  ...
  index do
    column :name
    column("View") {|row| link_to "View", admin_printer_function_path(row)}
    column("Edit") {|row| link_to "Edit", edit_admin_printer_function_path(row)}
    column "Delete" do |row|
      if row.can_delete?
        link_to "Delete", admin_printer_function_path(row), action: :delete
      end
    end
  end
  
  show do
    attributes_table_for printer_function do
      [:name].each do |attr|
         row attr
       end
    end
    panel "Used for the following installations" do
      table_for printer_function.installations do
